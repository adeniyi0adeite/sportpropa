...
def vote_home(request):
    ...
    for cat in all_categories.filter(category__in=player_categories):
        players = cat.players.all()  # Access players directly
        for player in players:
            player.total_votes = aggregate_votes(
            ...
            player.team = PlayerRegistration.objects.filter(player=player, competition=cat.competition).first().team
            
        player_categories_list.append({
            'category': cat,
            'players': players,  # Changed from nominees to players
...
def vote_winners(request):
    ...
    for cat in all_categories.filter(category__in=player_categories):
        ...
        player_categories_list.append({
            ...
            'winner': cat.winner if cat.winner else None,  # Correctly set winner
        })
    ...
    for cat in all_categories.filter(category__in=team_categories):
        ...
        team_categories_list.append({
            ...
            'hide_vote_count': cat.hide_vote_count,
            'close_date': cat.close_date,
            'winner': cat.get_winner() if cat.is_closed() else None,
        })
    ...
    context = {
        ...
        'team_categories': team_categories_list,
        'coach_categories': coach_categories_list,
        'latest_competition': competitions.last() if competitions else None,  # Optional: get latest competition
    }

    return render(request, 'vote_winners.html', context)
