...
(function () {
	...
	$(function () {
		...
		// ajaxForms Ajax subscription
		if (plugins.ajaxForms.length) {
			for (var i = 0; i < plugins.ajaxForms.length; i++) {
				var $form = $(plugins.ajaxForms[i]),
				...
				$inputs.each(function () {
					var $input = $(this);
					$input.attr('name', $input.attr('name') || $input.attr('id'));
				});
				...
				$form.on('submit', function (event) {
					...
					var targetMessageContainer = '#message-container';
		
					if (formId === 'forget-password-form') {
						targetMessageContainer = '#forget-password-message';
					} else if (formId === 'verify-code-form') {
						targetMessageContainer = '#verify-code-message';
					} else if (formId === 'resend-code-form') {
						targetMessageContainer = '#resend-code-message';
					} else if (formId === 'change-password-form') {
						targetMessageContainer = '#change-password-message';
					} else if (formId === 'contact-form') {
						targetMessageContainer = '#contact-message-container';
					} else if (formId === 'feedback-form') {
						targetMessageContainer = '#feedback-message-container';
					} else if (formId === 'registration-form') {
						targetMessageContainer = '#registration-message';
					} else if (formId === 'login-form') {
						targetMessageContainer = '#login-message';
					}
		
					var data = $form.serialize();
					var url = $form.attr('action');
		
					$(targetMessageContainer).empty().removeClass('active error-message success-message info-message');
					...
					$.ajax({
						...
						beforeSend: function(xhr) {
							...
							$(targetMessageContainer).html('').addClass('info-message');
						},
						success: function(response) {
							if (!response.success) {
								var errorMessage = response.error;
								$(targetMessageContainer).html(errorMessage).removeClass("info-message").addClass("error-message");
								setTimeout(function () {
									$(targetMessageContainer).removeClass("error-message").empty();
								}, 4000);
							} else {
								var successMessage = '';
								...
								$(targetMessageContainer).html(successMessage).removeClass("info-message").addClass("success-message");
								...
								if (response.redirect) {
									...
									$form[0].reset();
									setTimeout(function () {
										$(targetMessageContainer).removeClass("success-message").empty();
									}, 6000);
								}
							}
						},
						error: function(xhr) {
							var generalErrorMessage = 'Error - ' + xhr.status + ': ' + xhr.statusText;
							$(targetMessageContainer).html(generalErrorMessage).removeClass("info-message").addClass("error-message");
							setTimeout(function () {
								$(targetMessageContainer).removeClass("error-message").empty();
							}, 6000);
						}
